...
- **What this repo owns:** prompt overrides, repo-specific skills, domain code, `CONTEXT.md` (domain glossary), ADRs (scope and decisions of record; read first), `.env`.
- **Bumping the kit:** the kit dep is unpinned (tracks `main`), so `npm update claude-code-kit` pulls the latest. For the skill bundle, re-run `npx skills@latest add OdinIversen/claude-code-kit`. Scaffolding templates (Dockerfile, CONTEXT.md, etc.) are intentionally NOT auto-synced — they're seeds you customised after init.
