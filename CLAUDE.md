...
- **Bumping the kit:** the kit dep is unpinned (tracks `main`), so `npm update claude-code-kit` pulls the latest. Skills are user-level (`~/.claude/skills`), never vendored into repos. Scaffolding templates (Dockerfile, CONTEXT.md, etc.) are intentionally NOT auto-synced — they're seeds you customised after init.
